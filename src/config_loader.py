...
def load_config(config_file: Path = Path('../config/config_settings.yaml')) -> Dict[str, Any]:
    ...
    # Apply EDA-specific matplotlib/seaborn settings
    eda_plot_settings = config.get('eda', {}).get('plot_settings', {})
    if 'default_figsize' in eda_plot_settings:
        plt.rcParams['figure.figsize'] = eda_plot_settings['default_figsize']
    if 'font_size' in eda_plot_settings:
        plt.rcParams['font.size'] = eda_plot_settings['font_size']
    if 'style' in eda_plot_settings:
        plt.style.use(eda_plot_settings['style'])
    if 'palette' in eda_plot_settings:
        sns.set_palette(eda_plot_settings['palette'])
